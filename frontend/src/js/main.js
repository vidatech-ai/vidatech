...
const API = window.location.hostname === '192.168.2.1' ? 'http://192.168.2.1/cgi-bin/proxy' : 'https://vidatech-wifi-z55g.onrender.com';
...
async function loadUsers() {
  ...
  const data = await api('/api/payments/?limit=500&confirmed_at=' + monthStart + '&status=confirmed');
  if (!data) return;
