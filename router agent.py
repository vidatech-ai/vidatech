...
class ZLTRouter:
    ...
    async def _setup_url_filter(self):
        """Enable URL whitelist mode and set portal as the only allowed URL."""
        try:
            # Enable URL whitelist mode
            await self._post({
                'cmd': CMD_URL_MODE,
                'method': 'POST',
                'sessionId': self.session_id,
                ...
                'success': True,
                'datas': [
                    {'enableRule': True, 'acceptAll': False, 'ippro': 'IPV4'},
                    {'enableRule': True, 'acceptAll': False, 'ippro': 'IPV6'},
                ],
            })
            # Set portal as only allowed URL
            await self._post({
                'cmd': CMD_URL_FILTER,
                'method': 'POST',
                'sessionId': self.session_id,
                ...
                'datas': [
                    ...
                    },
                    {
                        ...
                        'enableLink': True,
                        'ippro': 'IPV4V6',
                        'url': 'vidatech-wifi.onrender.com',
                        'remark': 'backend',
                    },
                ],
            })
            logger.info('URL filter set: portal-only mode active')
        except Exception as e:
            logger.error(f'URL filter setup failed: {e}')
    ...
    async def _write_firewall_rules(self, rules: list) -> bool:
        # Always refresh token immediately before writing
        try:
            token_resp = await self.client.post(
                f'{self.base}/cgi-bin/http.cgi',
                json={'cmd': CMD_TOKEN_REFRESH, 'method': 'GET', 'sessionId': self.session_id},
            ...
            logger.warning(f'Token refresh before firewall write failed: {e}')
...
async def enforce_access(router: ZLTRouter, devices: list):
    """
    Unpaid devices → not in firewall rules (neutral) → URL filter allows only portal
    Paid/whitelisted devices → added to MAC whitelist → full internet
    """
    db = get_db()
    now = utcnow()
    new_rules = []
    ...
    for device in devices:
        ...
        try:
            # WHITELISTED_MACS in env — always full internet
            ...
            # Check if admin unblocked via dashboard
            ...
            if result.data:
                ...
                # Unpaid — no firewall rule, URL filter handles portal-only access
                logger.info(f'Unpaid device, portal-only: {mac}')
                db.table('devices').update({
                    'status': 'blocked',
                    'ip_address': ip,
                }).eq('mac_address', mac).execute()
    ...
    # Write ALL rules in ONE single call
    try:
